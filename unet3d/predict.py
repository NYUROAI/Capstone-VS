...
def volumetric_predictions(model_filename, filenames, prediction_dir, model_name, n_features, window,
                           ...
                           resample_predictions=True, output_template=None,output_prob_template = None, segmentation=False,
    ...
    if package == "pytorch":
        pytorch_volumetric_predictions(model_filename=model_filename,
                                       ...
                                       output_prob_template = output_prob_template,
...
def predict_volumetric_batch(model, batch, batch_references, batch_subjects, batch_filenames,
                             ...
                             segmentation, output_template, output_prob_template,n_gpus, verbose, threshold, interpolation,
    ...
    for batch_idx in range(len(batch)):
        #store prob matrix to output_prob_template
        pred_image_prob = new_img_like(batch_references[batch_idx][0],data = pred_x[batch_idx].squeeze())
        pred_image_prob = resample_to_img(pred_image_prob, batch_references[batch_idx][1],
                                     interpolation=interpolation)
        write_prediction_image_to_file(pred_image_prob, output_prob_template,
...
def pytorch_volumetric_predictions(model_filename, model_name, n_features, filenames, window,
                                   ...
                                   output_template=None, output_prob_template = None, segmentation=False, segmentation_labels=None,
        ...
        for idx in range(len(dataset)):
            ...
            if len(batch) >= batch_size or idx == (len(dataset) - 1):
                predict_volumetric_batch(model=model, batch=batch, batch_references=batch_references,
                                         ...
                                         segmentation=segmentation, output_template=output_template,output_prob_template = output_prob_template, n_gpus=n_gpus,
