...
def flatten(tensor):
    ...
    return transposed.contiguous().view(C, -1).cpu().detach().numpy()


def hausdorff(x, y):
    return torch.tensor(max(directed_hausdorff(flatten(x), flatten(y))[0], directed_hausdorff(flatten(y), flatten(x))[0]))
